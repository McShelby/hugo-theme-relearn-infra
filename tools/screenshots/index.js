#!/usr/bin/env node
//
// Regenerates the `featured.png` preview images for the shortcode pages in the
// theme's docs.
//
...
// theme the tests just verified.

import fs from 'fs';
import path from 'path';
import puppeteer from 'puppeteer';
import { serve } from '../../runner/hugo.js';
...
async function capture(browser, base, rel) {
  ...
  try {
    ...
    const clip = { x: 0, y: 0, width: iPhoneViewport.width, height: Math.floor(iPhoneViewport.height / 2) };
  ...
  // Create a preview page to apply CSS perspective and save
  try {
    ...
    // Write into the docs of the resolved theme checkout, never a path derived
    // from __dirname - this script does not live in the repository it writes to.
    const targetDir = path.join(docsDir, 'content', rel.replace(/^\//, ''));
...
async function run() {
  ...
  if (!base) {
    console.log(`Serving ${docsDir} on port ${port}`);
    server = serve({ siteDir: docsDir, themeDir, port, injectTheme: false });
    await server.ready;
    base = server.url;
  }
