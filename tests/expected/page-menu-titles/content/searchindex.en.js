var relearn_searchindex = [
  {
    "breadcrumb": "Page Menu Titles \u003e Both, from linkTitle",
    "content": "A page menu prints its title only when the tree below it has entries, so every root here needs a child.",
    "description": "A page menu prints its title only when the tree below it has entries, so every root here needs a child.",
    "tags": [],
    "title": "Both Child",
    "uri": "/both/child/index.html"
  },
  {
    "breadcrumb": "Page Menu Titles \u003e Linktitle, from linkTitle",
    "content": "A page menu prints its title only when the tree below it has entries, so every root here needs a child.",
    "description": "A page menu prints its title only when the tree below it has entries, so every root here needs a child.",
    "tags": [],
    "title": "Linktitle Child",
    "uri": "/linktitle/child/index.html"
  },
  {
    "breadcrumb": "Page Menu Titles",
    "content": "Root of the linktitle menu.",
    "description": "Root of the linktitle menu.",
    "tags": [],
    "title": "Link Title Section",
    "uri": "/linktitle/index.html"
  },
  {
    "breadcrumb": "Page Menu Titles \u003e Menu Title Section",
    "content": "A page menu prints its title only when the tree below it has entries, so every root here needs a child.",
    "description": "A page menu prints its title only when the tree below it has entries, so every root here needs a child.",
    "tags": [],
    "title": "Menutitle Child",
    "uri": "/menutitle/child/index.html"
  },
  {
    "breadcrumb": "Page Menu Titles \u003e Neither Section",
    "content": "A page menu prints its title only when the tree below it has entries, so every root here needs a child.",
    "description": "A page menu prints its title only when the tree below it has entries, so every root here needs a child.",
    "tags": [],
    "title": "Neither Child",
    "uri": "/neither/child/index.html"
  },
  {
    "breadcrumb": "Page Menu Titles",
    "content": "Root of the menutitle menu.",
    "description": "Root of the menutitle menu.",
    "tags": [],
    "title": "Menu Title Section",
    "uri": "/menutitle/index.html"
  },
  {
    "breadcrumb": "Page Menu Titles",
    "content": "Root of the both menu.",
    "description": "Root of the both menu.",
    "tags": [],
    "title": "Both Section",
    "uri": "/both/index.html"
  },
  {
    "breadcrumb": "Page Menu Titles",
    "content": "Root of the neither menu.",
    "description": "Root of the neither menu.",
    "tags": [],
    "title": "Neither Section",
    "uri": "/neither/index.html"
  },
  {
    "breadcrumb": "Page Menu Titles",
    "content": "",
    "description": "",
    "tags": [],
    "title": "Categories",
    "uri": "/categories/index.html"
  },
  {
    "breadcrumb": "",
    "content": "Four page menus in the sidebar, each rooted at one of the sections below. What each one prints above its tree is the whole point of this fixture.",
    "description": "Four page menus in the sidebar, each rooted at one of the sections below. What each one prints above its tree is the whole point of this fixture.",
    "tags": [],
    "title": "Page Menu Titles",
    "uri": "/index.html"
  },
  {
    "breadcrumb": "Page Menu Titles",
    "content": "",
    "description": "",
    "tags": [],
    "title": "Tags",
    "uri": "/tags/index.html"
  }
]
